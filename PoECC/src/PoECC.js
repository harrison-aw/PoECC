...
function PoEButton(props) {
	let className = typeof props.className === 'undefined' ? 'PoEButton' : 'PoEButton ' + props.className;
	return (
		<button className={className} type="button" onClick={() => props.onClick()}>{props.label}</button>
	);
}
...
class PoENavigation extends Component {
	...
	render() {
		let buttons = PoENavigation.buttonLabels().map((label) => <PoEButton className={'nav'+label} key={label} label={label} onClick={() => this.props.changePanel(label)} />);
		return (
